...
export default async (bot = initBot(), message) => {
  const breakoutChannels = await airGet('Breakout Channel', 'Open', true)
    ...
    bot.api.conversations.history(
      ...
      (err, res) => {
        ...
        if (latestTimestamp == breakout.fields['Last Updated Timestamp']) {
          bot.replyInThread(message, `Closing <#${breakout.fields["Breakout Channel ID"]}>`)
          airPatch('Breakout Channel', breakout.id, {
            'Open': false
          })
          bot.api.conversations.rename({ channel: breakout.fields['Breakout Channel ID'], name: 'archived-' + breakout.fields['Breakout Channel Name']}, (err, res) => {
            if (err) {/* do something */}
            bot.api.conversations.archive({channel: breakout.fields['Breakout Channel ID']}, (err, res) => {
              if (err) {/* do something */}
          ...
          bot.replyInThread(message, `Keeping open <#${breakout.fields["Breakout Channel ID"]}>`)
          airPatch('Breakout Channel', breakout.id, {
