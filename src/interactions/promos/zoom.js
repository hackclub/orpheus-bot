...
export const details = 'Available for one club leader at each club (the point of contact). They can add their co-leaders and members to their account.'
export async function run(bot, message) {
  ...
  if (!leader.fields['Zoom ID']) {
    ...
    bot.replyPrivateDelayed(
      message,
      transcript('promos.zoom.createdAccount', { email: leader.fields['Email'] })
    )
  } else {
    ...
    if (!zoomAccount.type) {
      ...
      bot.replyPrivateDelayed(message, transcript('promos.zoom.pendingAccount', { email: leader.fields['Email'] }))
    } else {
      // toggle account type
