  ...
  bot.startPrivateConversation(message, (err, convo) => {
    ...
    getInfoForUser(message.user).then(({
      ...
      if (history.meetings.length > 0) {
        const lastMeetingDay = new Date(history.meetings[0].fields['Date']).toLocaleDateString('en-us', { weekday: 'long', timeZone: slackUser.tz })
      ...
      convo.addQuestion({
      ...
      }, [
        {
          ...
          callback: (response, convo) => {
            ...
            // attempt to parse
            const meetingDate = parseDate(`${response.text} ${slackUser.tz}`)
            if (Object.prototype.toString.call(meetingDate) === '[object Date]') {
              ...
              convo.setVar('date', {
                full: meetingDate,
                dayName: meetingDate.toLocaleDateString('en-us', { weekday: 'long', timeZone: slackUser.tz }),
                mmddyyyy: meetingDate.toLocaleDateString('en-us', {timeZone: slackUser.tz})
              })
              convo.say({
